class Planet {
	...
	// orbit() {
	// 	//for (var i = 0; i = 100; i++) {
	// 	this.d = int(dist(planets[0].x, planets[0].y, planets[1].x, planets[1].y) * 1000);
	// 	if (planets[1].x >= planets[0].x && planets[1].y <= planets[0].y) { //1st Quad
	// 		planets[1].ax = -0.08 * ((planets[0].mu) / (this.d ^ 3)) * planets[1].x;
	// 		planets[1].ay = 0.08 * ((planets[0].mu) / (this.d ^ 3)) * planets[1].y;
	// 	} else if (planets[1].x <= planets[0].x && planets[1].y <= planets[0].y) { //2nd Quad
	// 		planets[1].ax = 0.08 * ((planets[0].mu) / (this.d ^ 3)) * planets[1].x;
	// 		planets[1].ay = 0.08 * ((planets[0].mu) / (this.d ^ 3)) * planets[1].y;
	// 	} else if (planets[1].x <= planets[0].x && planets[1].y >= planets[0].y) { //3rd Quad
	// 		planets[1].ax = 0.08 * ((planets[0].mu) / (this.d ^ 3)) * planets[1].x;
	// 		planets[1].ay = -.08 * ((planets[0].mu) / (this.d ^ 3)) * planets[1].y;
	// 	} else { //4th Quad
	// 		planets[1].ax = -.08 * ((planets[0].mu) / (this.d ^ 3)) * planets[1].x;
	// 		planets[1].ay = -.08 * ((planets[0].mu) / (this.d ^ 3)) * planets[1].y;
	// 	}
	// 	print("Ax:", planets[1].ax);
	// 	print("Ay:", planets[1].ay);
	...
	trail() {
		...
		this.colorC = int(map(this.d / 1000, 0, width / 2, 0, 255));
